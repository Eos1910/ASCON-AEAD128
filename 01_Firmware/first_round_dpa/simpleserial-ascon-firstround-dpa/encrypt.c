...
/*
 * First-round DPA/CPA helper.
 *
 * This helper is for the traditional fixed-key random-nonce DPA/CPA baseline.
 * It captures the first ASCON initialization round, where the state contains:
 *
 *   x0 = ASCON IV
 *   x1 = K0
 *   x2 = K1
 ...
 *   x4 = N1
 *
 * The trigger is placed only around the first P12 round:
 *
 *   ROUND(s, 0xf0)
 *
 * Then the remaining 11 rounds are completed outside the trigger so that the
 * state evolution remains close to normal ASCON initialization.
 */
uint8_t ascon_first_round_dpa_capture(
    const uint8_t *key,
    const uint8_t *nonce
)
{
    ...
    /*
     * Same initialization state as your existing ASCON implementation.
     */
    s.x[0] = ASCON_128A_IV;
    s.x[1] = K0;
    s.x[2] = K1;
    ...
    /*
     * Capture only the first ASCON initialization round.
     */
    trigger_high();
    ROUND(&s, 0xf0);
    trigger_low();
    ...
    /*
     * Finish the remaining 11 rounds outside the trigger.
     * This keeps the computation close to real ASCON initialization,
     * while keeping the leakage window focused on the first round.
     */
    ROUND(&s, 0xe1);
    ROUND(&s, 0xd2);
    ROUND(&s, 0xc3);
    ...
    /*
     * Final key injection, same as your ascon_initialize().
     */
    s.x[3] ^= K0;
    s.x[4] ^= K1;

    /*
     * Checksum prevents optimization and confirms execution.
     * This is not secret output and not used as ciphertext.
     */
    checksum = 0;
    checksum ^= (uint8_t)(s.x[0]);
    checksum ^= (uint8_t)(s.x[0] >> 8);
    checksum ^= (uint8_t)(s.x[1] >> 16);
    checksum ^= (uint8_t)(s.x[2] >> 24);
    checksum ^= (uint8_t)(s.x[3] >> 32);
    checksum ^= (uint8_t)(s.x[4] >> 40);
